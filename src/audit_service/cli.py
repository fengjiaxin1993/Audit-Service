...
from audit_service.startup import main as startup_main
from audit_service.init_database import create_tables
...
def init():
    ...
    if (bs.PACKAGE_ROOT / "data/frontend" != Path(bs.DATA_PATH) / "frontend"):
        shutil.copytree(bs.PACKAGE_ROOT / "data/frontend", Path(bs.DATA_PATH) / "frontend",
                        dirs_exist_ok=True)
    logger.success("复制 HTML文件：成功。")
